...
import { mockClients, mockProducts } from '@/data/mock';
...
export function InvoiceForm() {
  ...
  const addLine = () => {
    setLines([...lines, { id: Date.now().toString(), productId: '', quantity: 1, unitPrice: 0, discount: 0, total: 0 }]);
  };
    ...
    setLines(lines.map(line => {
      if (line.id === id) {
        ...
        if (field === 'productId') {
          ...
          if (product) {
            newLine.unitPrice = product.salePrice;
            newLine.quantity = 1;
          }
        }
    ...
    <div className="bg-surface rounded-xl shadow-sm border border-border p-6 space-y-8">
      ...
      <div className="space-y-4">
        ...
        {lines.map((line) => {
          ...
          return (
          <div key={line.id} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center bg-background-subtle p-4 rounded-lg md:bg-transparent md:p-0 md:rounded-none transition-all duration-200">
            ...
            <div className="md:col-span-2 space-y-1">
               ...
               <input 
                  ...
                  className="w-full rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text-primary focus:border-primary focus:outline-none"
                />
                {product && line.quantity > maxQty && (
                  <p className="text-xs text-red-500 font-medium">Max dispo: {maxQty}</p>
                )}
            </div>
            ...
            <div className="md:col-span-2 flex items-center justify-between md:justify-end gap-2 mt-2 md:mt-0">
               ...
               <button 
                 ...
                 className="p-2 text-text-muted hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors active:scale-95"
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 pt-6 border-t border-border">
        <div className="w-full md:w-auto flex flex-col sm:flex-row gap-3 order-2 md:order-1">
          {error && (
            <div className="flex items-center gap-2 px-3 py-2 bg-red-50 border border-red-200 rounded-lg text-red-600 text-sm sm:col-span-2 w-full">
          ...
          <button
            ...
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-primary text-white hover:bg-[#0D5C4A] hover:shadow-md transition-all duration-200 active:scale-95"
          >
            <Save className="w-4 h-4" /> Enregistrer
          </button>
          <button
            onClick={() => handleSave('envoyee')}
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 rounded-lg border border-border text-text-secondary hover:bg-background-subtle hover:text-text-primary transition-all duration-200 active:scale-95 hover:shadow-sm"
          >
