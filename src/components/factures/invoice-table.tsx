...
type Invoice = typeof mockInvoices[number] & { status: InvoiceStatus };
...
function StatusDropdown({
  ...
  return (
    <div
      ref={ref}
      className="absolute z-50 top-full left-0 mt-1.5 bg-white border border-gray-200 rounded-xl shadow-lg py-1.5 w-44"
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className={`font-medium ${current === s ? "text-gray-900" : "text-gray-600"}`}>
                {cfg.label}
              </span>
            </div>
...
export function InvoiceTable() {
  ...
  const [invoices, setInvoices] = useState<Invoice[]>(
    mockInvoices.map((inv) => ({ ...inv, status: inv.status as InvoiceStatus }))
  );
            ...
            {filtered.length > 0 ? (
              filtered.map((inv) => {
                ...
                return (
                  <tr
                    key={inv.id}
                    className="group hover:bg-background-subtle/50 transition-colors cursor-pointer"
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-text-primary font-medium">{inv.clientName}</td>
                    <td className="px-5 py-4 text-text-secondary">{formatDate(inv.date)}</td>
                    <td className="px-5 py-4 text-text-secondary">{formatDate(inv.dueDate)}</td>
                    ...
                    <td className="px-5 py-4">
                      ...
                      <div className="relative inline-block">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setOpenStatusId(isOpen ? null : inv.id);
                          }}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/factures/${inv.id}?pdf=1`);
                          }}
                          className="p-1.5 text-text-muted hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors hover:scale-110 active:scale-95"
              ...
              <tr>
                <td colSpan={7} className="px-5 py-16 text-center">
                  <div className="flex flex-col items-center gap-3 text-text-muted">
                    ...
                    <p className="text-xs">Essayez de modifier vos filtres ou créez une nouvelle facture.</p>
                  </div>
                </td>
              </tr>
            )}
