import { mockStockAlerts } from '@/data/mock';
    ...
    <div className="bg-surface border border-border rounded-xl shadow-sm p-6 flex flex-col h-[300px]">
      <div className="flex items-center justify-between mb-6">
        ...
        <span className="text-xs font-medium bg-danger/10 text-danger px-2 py-1 rounded-full">
          {mockStockAlerts.length} articles
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-3">
        {mockStockAlerts.length > 0 ? (
          mockStockAlerts.map((item) => (
            <div key={item.id} className="flex items-center justify-between p-3 rounded-lg border border-border bg-background-subtle gap-2">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-text-primary leading-tight mb-0.5 truncate" title={item.name}>{item.name}</p>
                  <p className="text-xs text-text-secondary truncate" title={item.category}>{item.category}</p>
                </div>
              </div>
              <div className="text-right shrink-0">
                <p className="text-lg font-bold text-danger">{item.quantity}</p>
                <p className="text-xs text-text-secondary hidden sm:block">en stock (min {item.alertThreshold})</p>
